...
def banner(checkpoint, output_dir, task):
    lines = [
        ...
        "      wr.start()",
        "",
        "  start() returns immediately and runs on Kit's update loop. The first",
        "  call builds the env and loads the policy (~30 s); later calls reuse",
        "  both. The result line lands in the Console window, not here.",
        "",
